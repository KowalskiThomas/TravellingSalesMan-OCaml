module Optimizer = struct
    ...
    *)
    ...
    let rec repositionnement_n_fois solution n c =
        if n = 0 
        then solution
        else
            let city, _ = Carte.get_random_any c in
            let swapped = inversion_locale city solution c in
            repositionnement_n_fois swapped (n - 1) c

    let rec build_solution_nearest path count max_cities carte = 
        if count = max_cities 
        then path 
        else
            let new_path = MLLPath.insert_nearest_minimize_length path carte in
            build_solution_nearest (new_path) (count + 1) max_cities carte

    let find_solution_nearest carte = 
        let idx_start, _ = Carte.get_random_any carte in
        let chemin = MLLPath.make idx_start in 
        let max_cities = Carte.card carte in 

        let solution = build_solution_nearest chemin 1 max_cities carte in
