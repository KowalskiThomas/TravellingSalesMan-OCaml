...
let noError = Error ("No Exception", EmptySet)
...
let tests_bool =
  [
    ...
    "equal_order", (fun () -> equal (add 3 (add 2 empty)) (add 2 (add 3 empty))), true, fprintf_bool;

    "doublons", (fun () -> equal (add 3 (add 2 (add 3 empty))) (add 3 (add 2 empty))), true, fprintf_bool;

    (* "union",(fun() -> equal
      (add 3 (add 2 (add 1 empty)))
      (union
        (add 3 (add 2 empty))
        (add 2 (add 1 empty))
      )), true, fprintf_bool; *)
...
let tests_exn =
  [
    ...
    "no_exception_mem_empty", (fun() ->
      try
        ignore(mem 3 empty);
        noError
      with e ->
        if e = EmptySet
          then Error("Shouldn't Raise", EmptySet)
          else noError
      ), noError, fprintf_exn;
    (* "mem_empty", (fun () ->
      try
